...
class SimpleGladeApp:
    ...
    def __repr__(self):
        ...
        if self.main_widget:
            ...
            repr = '%s(path="%s", root="%s")' % (
                class_name, self.glade_path, root)
        else:
        ...
        for widget in self.builder.get_objects():
            ...
            if hasattr(self, widget_api_name):
                raise AttributeError(
                    "instance %s already has an attribute named %s" % (self, widget_api_name))
            else:
    ...
    def add_prefix_actions(self, prefix_actions_proxy):
        ...
        def is_method(t): return callable(t[1])
        def is_prefix_action(t): return t[0].startswith(prefix_s)
        def drop_prefix(t): return (t[0][prefix_pos:], t[1])
