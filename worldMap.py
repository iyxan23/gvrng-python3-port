...
#     print("in gvrparser locale switch:\n", info, file=sys.stderr)
# _ = gettext.gettext
KEYWORDS = (
    'ROBOT',
    'WALL',
...
def readWorld(lines, world):
        ...
        try:
            if re.search(r"\S", line) and not re.match(r"\s*#", line):
                ...
                if keyword == 'WALL':
                    ...
                    if definedRobot:
                        raise WorldMapException(linenumber,
                                                'You may only have one robot definition.')
                    definedRobot = 1
                    tokens = [x.upper() for x in tokens]
                    if len(tokens) == 5:
                    ...
                    if worldSize:
                        raise WorldMapException(linenumber,
                                                'You may only have one size statement')
                    try:
                        avenues, streets = [int(coord) for coord in tokens[1:]]
                    except ValueError:
                        raise WorldMapException(linenumber,
                                                'Size statement should have 2 integers')
                    if avenues < 7 or streets < 7:
                        raise WorldMapException(linenumber,
                                                'Size coordinates must be at least 7')
                    worldSize = (avenues, streets)
                else:
                    raise WorldMapException(linenumber,
                                            "Cannot understand: %s" % line)
        except WorldMapException as e:
    ...
    if not definedRobot:
        raise WorldMapException(linenumber,
                                "The world map seems to be missing information.")
