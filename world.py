...
class World:
    ...
    def positionRobot(self, x, y, dir):
        if x < 1:
            raise WorldMapException(
                "Bad x value for positioning robot: %s" % x)
        if y < 1:
            raise WorldMapException(
                "Bad y value for positioning robot: %s" % y)
        self.robot = (x, y)
        self.dir = dir
...
# return (shiftX or shiftY, (offsetX, offsetY))
    ...
    def newOffset(self, offsets: (int, int), size: (int, int)):
