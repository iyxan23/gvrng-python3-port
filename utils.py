...
if platform == 'XO':
    ...
    PROGRAMSDIR = os.path.join(
        os.environ['SUGAR_ACTIVITY_ROOT'], 'data', 'GvR')
    GTKSOURCEVIEWPATH = os.path.join(
        os.environ['SUGAR_ACTIVITY_ROOT'], 'data', '.gnome2/gtksourceview-1.0/language-specs')
else:
    RCFILE = os.path.expanduser('~/.gvrngrc')
    PROGRAMSDIR = os.path.join(os.path.expanduser('~'), 'GvR')
    GTKSOURCEVIEWPATH = os.path.join(os.path.expanduser(
...
if os.name == 'nt' and GVRREDIRECT:
    try:
        ...
        print("Redirect sys.stderr", file=sys.stderr)
    ...
    try:
        ...
        if 'en' in lang:
            ...
            module_logger.exception(
                "Error in setting the locale, switching to English")
            lang = 'en'
    return lang
...
def trace_error(file=sys.stderr):
    ...
    print(' Please send a bug report with this stuff to,\n stas.zytkiewicz@gmail.com', file=file)
    print('*'*60, file=file)
...
def parseRcFile():
    ...
    try:
        ...
        print(info, "Setting language and printer to defaults.", file=sys.stderr)
        print("=========================================================")
        print("The configuration file used by GnRng is changed.")
        print("You should remove %s and restart GvRng." %
              os.path.expanduser('~/.gvrngrc'))
